<div>
    <main class="flex flex-col items-center justify-center h-dvh gap-6 bg-gym-hero">
        ...
        <section class="w-11/12 {{ $step === 3? 'max-w-2xl' : 'max-w-md'}} bg-white">
            <form wire:submit="{{ $step === 1 ? 'sendOtp' : ($step === 2 ? 'verifyOtp' : 'recoveryPassword') }}" class="flex flex-col border-2 p-6 gap-4 shadow-brutal items-center">
                <div wire:key="recovery-step-{{ $step }}" class="w-full step-slide-left">
                    @if($step === 1)
                        @include('auth.recovery.get-email')
                    @elseif($step === 2)
                        @include('auth.recovery.get-otp')
                    @elseif($step === 3)
                        @include('auth.recovery.get-password')
                    @endif
                </div>
            </form>
        </section>
